...
export default function Dashboard() {
    ...
    const handleSubmitShipment = () => {
        if (!newStopData.lat || !newStopData.lng || !newStopData.name) {
            alert('Lütfen konum ve müşteri adı giriniz.');
            return;
        }
        ...
        const newOrder = {
            ...
            location: [parseFloat(newStopData.lat), parseFloat(newStopData.lng)],
            load: parseInt(newStopData.load) || 0,
            deliveryTime: newStopData.deliveryTime,
            notes: newStopData.notes,
            status: 'pending',
            submissionTime: new Date().toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })
        };

        if (isWorkingHours()) {
            // 09:00 - 18:00 -> Send to Approval Queue
            setApprovalQueue(prev => [...prev, { ...newOrder, status: 'waiting_approval' }]);
            alert('Saat 09:00-18:00 arası olduğu için sevkiyat YÖNETİCİ ONAYINA gönderildi.');
        } else {
            // Before 09:00 -> Direct to Pool
    ...
    const handleAssignOrder = async (order, targetVehicleId) => {
        ...
        if (targetVehicle.currentLoad + order.load > targetVehicle.capacity) {
            if (!confirm(`UYARI: Kapasite aşılacak! Devam?`)) return;
        }

        let startPoint = targetVehicle.location;
        if (targetVehicle.route.length > 0) {
            const last = targetVehicle.route[targetVehicle.route.length - 1];
            startPoint = (Array.isArray(last) && !Array.isArray(last[0])) ? last : last.coordinates[last.coordinates.length - 1];
        }

        try {
            const routeData = await getRoute(startPoint, order.location);
            setVehicles(prev => prev.map(v => {
                if (v.id === targetVehicleId) {
                    return {
                        ...v,
                        routeSegments: [...(v.routeSegments || []), { coordinates: routeData.coordinates, color: routeData.color }],
                        route: [...v.route, ...routeData.coordinates],
                        nextStop: order.customer,
                        lastTripInfo: `${routeData.distance} km / ${routeData.duration} dk`,
                        currentLoad: v.currentLoad + order.load
                    }
                }
                return v;
            }));
            setPendingOrders(prev => prev.filter(o => o.id !== order.id));
            alert('Sevkiyat atandı!');
